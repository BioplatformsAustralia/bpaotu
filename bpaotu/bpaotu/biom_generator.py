...
for idx, row in enumerate(csv_data[1:]): # Start from the second row (first row is the header)
    ...
    x = otu_codes.index(otu_code) # Get the column number of the OTU code (which is the x value)
    y = bpa_sample_ids.index(bpa_sample_id) # Get the row number of the BPA ID (which is the y value)
    z = row[header_index['otu_count']] # Get the count from the CSV file

    data_matrix.append([x, y, z])
...
def _format_col_data(list, metadata=None):
...
biom_file['rows'] = formatted_sample_ids
biom_file['columns'] = formatted_otu_codes
biom_file['matrix_type'] = "sparse"
biom_file['matrix_element_type'] = "int"
biom_file['shape'] = [len(formatted_sample_ids), len(formatted_otu_codes)]
