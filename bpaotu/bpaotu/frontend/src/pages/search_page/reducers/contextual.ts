...
export const doesFilterMatchEnvironment = environment => filter => {
    ...
    const eq = filter => filter.environment == environment.value;
    const op = environment.operator === 'is' ? eq : _.negate(eq);
    return _.isNull(filter.environment) || op(filter);
}
...
const contextualFiltersReducer = handleActions({
    ...
    [combineActions(selectEnvironment, selectEnvironmentOperator) as any]: (state, action) => ({
        ...state,
        filters: _.map(state.filters, f => {
            if (f.name === '') {
                return f;
            }
            const dataDefinition = _.find(state.dataDefinitions.filters, dd => dd.name === f.name);
