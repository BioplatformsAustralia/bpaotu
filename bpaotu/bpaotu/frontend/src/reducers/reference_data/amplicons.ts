...
export default handleAction(fetchAmpliconsEnded, {
    next: (state, action: any) => ({
        isLoading: false,
        error: false,
        values: _.map(action.payload.data.possibilities, (option: any) => ({id: option[0], value: option[1]})),
    }),
