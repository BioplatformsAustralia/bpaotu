...
export const { setMetagenomeMode, selectAmplicon, selectAmpliconOperator } = createActions(
    'SET_METAGENOME_MODE', 'SELECT_AMPLICON', 'SELECT_AMPLICON_OPERATOR')


function lookupAmplicon(values, ampliconName) {
  const defaultAmplicon = find(
    values,
    amplicon => amplicon.value === ampliconName)
  return (defaultAmplicon) ? defaultAmplicon : (values.length) ? values[0] : undefined
}

export const getDefaultAmplicon = (values) => lookupAmplicon(
  values,
  window.otu_search_config.default_amplicon)

export const getDefaultMetagenomeAmplicon = (values) => lookupAmplicon(
  values,
  window.otu_search_config.metaxa_amplicon)
