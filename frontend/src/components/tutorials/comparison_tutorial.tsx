...
const comparisonTourSteps = (props, isMainTourOpen) => {
  return [
    {
      content: () => {
        return (
          <div>
            <h4>Interactive Sample Comparison Tutorial</h4>
            <p>
              The Interactive Sample Comparison tool can be used to visually interrogate the AM
              dataset.
            </p>
...
const ComparisonTutorial = (props) => {
  ...
  const steps = comparisonTourSteps(props, isMainTourOpen)
  ...
  return (
    <>
      <Tutorial
        steps={steps}
        startAt={comparisonTourStep}
        isOpen={isComparisonTourOpen}
        getCurrentStep={(curr) => setComparisonTourStep(curr)}
        onRequestClose={() => {
      ...
      />
      <TutorialBadge
        id="badgeComparisonTutorial"
        onClick={() => {
          setIsComparisonSubtourOpen(false)
          setIsComparisonTourOpen(true)
          track('otu_tutorial_comparison_open')
        }}
        tooltip="This tutorial helps to use interactive sample comparison feature in the BPA-OTU data portal"
      />
    </>
  )
}
