...
export default class ContextualFilter extends ContextualDropDown {
  ...
  public componentDidMount() {
    if (this.selectRef.current) {
      ...
      this.props.changeValue(
        this.props.index,
        this.selectRef.current.props.value
      );
    }
  }
  ...
  protected renderOperatorAndValue() {
    ...
    const filterTipId =
      "FilterTip-" + this.props.index + "-" + this.props.filter.name;
          ...
          <>
              ...
              <UncontrolledTooltip target={filterTipId} placement="auto">
                {
                  find(
                    ...
                    (def) => def.name === this.props.filter.name
                  ).definition
                }
              </UncontrolledTooltip>
            </Col>
            <Col sm={7}>
              <TypeBasedOperatorAndValue
                ...
                changeOperator={(op) =>
                  this.props.changeOperator(this.props.index, op)
                }
                changeValue={(value) =>
                  this.props.changeValue(this.props.index, value)
                }
                changeValue2={(value) =>
                  this.props.changeValue2(this.props.index, value)
                }
                changeValues={(value) =>
                  this.props.changeValues(this.props.index, value)
                }
...
function StringOperatorAndValue({
  filter,
  dataDefinition,
  changeOperator,
  changeValue,
    ...
    <Row>
      <Col sm={4} className="no-padding-right">
        <Input
          ...
          onChange={forTargetValue(changeOperator)}
        >
...
function DropDownOperatorAndValue({
  ...
  const onChange = (evt) => {
    if (isMultiSelect) {
      const values = _filter(evt.target.options, (o) => o.selected).map(
        (o: any) => o.value
      );
      changeValues(values);
    } else {
